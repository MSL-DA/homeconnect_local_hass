...
- **Button** (`HCButtonEntityDescription`): `press_value_fn` — a callable returning the
  value to write when the button is pressed. Without it the button writes `True` to its
  `Command`, which is what a command-backed button needs. A button backed by a `Setting`
  sets this instead, and it is evaluated on press, so the value can depend on the current
  state (the clock button sends the current time this way).
