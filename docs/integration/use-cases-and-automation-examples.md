...
An appliance cut off from the internet - cloud disabled, or blocked at the router - can't sync its clock and drifts by a few seconds a day. This automation sets the clock from Home Assistant once a night, so it stays right and picks up DST changes on its own:
